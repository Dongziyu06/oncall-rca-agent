...
and not behaving correctly. This is
not
a guide for people who want to debug their cluster.
...
hostPort
there are a
limited number of places that pod can be scheduled. In most cases,
hostPort
...
Waiting
state, then it has been scheduled to a worker node,
but it can't run on that machine. Again, the information from
kubectl describe ...
should be informative. The most common cause of
Waiting
pods is a failure to pull the image.
...
If a Pod is stuck in the
Terminating
state, it means that a deletion has been
issued for the Pod, but the control plane is unable to delete the Pod object.
This typically happens if the Pod has a
finalizer
and there is an
admission webhook
...
this may be a
false
alarm, see https://github.com/kubernetes/kubernetes/issues/6842
pods/mypod
...
the apiserver makes one or more
EndpointSlice
resources available.
...
.
Last modified August 24, 2026 at 11:13 AM PST:
Fix Service selector example in Debug Pods (f209729935)
