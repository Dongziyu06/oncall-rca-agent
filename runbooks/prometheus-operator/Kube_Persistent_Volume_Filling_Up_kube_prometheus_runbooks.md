---
id: Kube_Persistent_Volume_Filling_Up_kube_prometheus_runbooks
category: kubernetes
fault_type: disk
severity: warning
source: prometheus-operator-runbooks
source_url: https://runbooks.prometheus-operator.dev/runbooks/kubernetes/kubepersistentvolumefillingup/
---

# Kube Persistent Volume Filling Up | kube-prometheus runbooks
...
minio
Notice
: some services may require special scaling conditions such as
...
`
kubectl -n <my-namespace> get pvc <my-pvc> -ojson | jq -r
...
kubernetes.io/gce-pd   Delete          Immediate           true                   28d
In this case
ALLOWVOLUMEEXPANSION
is true, so we can make use of the feature.
To resize the volume run:
$ kubectl -n <my-namespace> edit pvc <my-pvc>
And edit
.spec.resources.requests.storage
to the new desired storage size.
...
$ kubectl -n <my-namespace> delete pod
`
kubectl -n <my-namespace> get pod -ojson | jq -r
...
WARNING/DANGER
: This will permanently delete the data on the volume.
